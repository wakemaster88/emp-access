...
import { runAutomationTick } from "@/lib/shelly-automation";
...
export async function GET(request: NextRequest) {
  ...
  if (!authResult.ok) {
    console.warn(`[cron shelly-automations] Auth failed:`, JSON.stringify(authResult.body));
  ...
  try {
    ...
    console.log(
      `[cron shelly-automations] checked=${tick.checked} triggered=${tick.triggered}`
    );
    return NextResponse.json(tick);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[cron shelly-automations] failed:`, msg);
